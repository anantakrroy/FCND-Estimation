...
V3F QuadControl::BodyRateControl(V3F pqrCmd, V3F pqr)
{
  ...
  momentCmd = momentI * kpPQR * err_rate ; // (pqrCmd-pqr) is the rate error
...
//  float acc_z_cmd = p + i + d + accelZCmd;
  ...
  //From the quad dynamics
//
//   float z_dot_dot = (acc_z_cmd - CONST_GRAVITY) / b_z;
//   thrust = mass * CONSTRAIN(z_dot_dot, -maxAscentRate/dt, maxAscentRate/dt);
